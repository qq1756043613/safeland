...
if(FIFO_TYPE == 1) begin : NORMAL_FIFO
    ...
    // reg declaration
    reg [DATA_HAM_WIDTH - 1:0] mem [0:FIFO_DEPTH - 1];
    ...
    // combinational logic
    // -- Common
    assign data_ecd_o       = mem[rd_addr_map];
    // -- Write handle
    ...
    // Localparameter
    localparam CAT_NUM    = DATA_HAM_WIDTH/DATA_HAM_WIDTH;
    ...
    wire sml_full;
    // -- reg
    reg [DATA_HAM_WIDTH-1:0] buffer [0:CAT_NUM-1];
    ...
    for(sml_idx = 0; sml_idx < CAT_NUM; sml_idx = sml_idx + 1) begin : OUT_FLAT
        if(CONCAT_ORDER == "LSB") begin
            assign data_ecd_o[DATA_HAM_WIDTH*(sml_idx+1)-1:DATA_HAM_WIDTH*sml_idx] = buffer[sml_idx];
        end
        else if(CONCAT_ORDER == "MSB") begin
            assign data_ecd_o[DATA_HAM_WIDTH*(sml_idx+1)-1:DATA_HAM_WIDTH*sml_idx] = buffer[CAT_NUM - 1 - sml_idx];
        end
    end
    ...
    wire wr_hsk;
    wire rd_hsk;
    wire [DATA_HAM_WIDTH-1:0] data_map [0:CAT_NUM-1];
    wire buf_ocp;
    // -- reg
    reg [DATA_HAM_WIDTH-1:0] buffer;
    reg [CAT_NUM_W-1:0] sml_cnt;
    reg rd_ptr;
    reg wr_ptr;

    // Combination logic
    assign data_ecd_o       = data_map[sml_cnt];
    ...
    for(sml_idx = 0; sml_idx < CAT_NUM; sml_idx = sml_idx + 1) begin : BUF_MAP
        if(DECONCAT_ORDER == "LSB") begin
            assign data_map[sml_idx] = buffer[DATA_HAM_WIDTH*(sml_idx+1)-1:DATA_HAM_WIDTH*sml_idx];
        end
        else if(DECONCAT_ORDER == "MSB") begin
            assign data_map[CAT_NUM-1 - sml_idx] = buffer[DATA_HAM_WIDTH*(sml_idx+1)-1:DATA_HAM_WIDTH*sml_idx];
        end
    end
